...
public abstract class FormatHandler<T>
{
    ...
    public virtual FormatMetadata Metadata         => throw new NotImplementedException();

    /// <summary>Basic capabilities of the file format the handler is for</summary>
    public virtual FormatCapabilities Capabilities => throw new NotImplementedException();
    ...
    public bool Load(byte[] finBuffer, in T resource, ResourceParameters parameters = null)
    {
        // Simply create a file input stream and pass it to the implemented method
        using FileInputStream finStream = new(finBuffer);

        return Load(finStream, resource, parameters);
    }
    ...
    /// <param name="result">The resulting data from parsing</param>
    /// <returns>True on success, false otherwise.</returns>
    public bool Load(string finPath, in T resource, ResourceParameters parameters = null)
    {
        // Simply create a file input stream and pass it to the implemented method
        using FileInputStream finStream = new(finPath);

        return Load(finStream, in resource, parameters);
    }
    ...
    /// <param name="result">The resulting data from parsing</param>
    /// <returns>True on success, false otherwise.</returns>
    public virtual bool Load(FileInputStream finStream, in T resource, ResourceParameters parameters = null) =>
