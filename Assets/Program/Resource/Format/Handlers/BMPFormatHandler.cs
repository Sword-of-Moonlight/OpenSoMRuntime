...
public class BMPFormatHandler : FormatHandler<TextureResource>
{
        ...
        unchecked   // We don't need overflow checking at the moment, thanks. gimmie the perf.
        {
            switch (bitsPerPixel)
            {
                ...
                case 24 or 32:
                    ...
                    // We read rows so we don't choke the disc
                    Span<byte> rowBuffer = stackalloc byte[bytesPerRow + paddingPerRow];    // We also read the padding into this, even though it isn't used. Why not, eh?

                    for (int y = 0; y < height; ++y)
                    {
                        // Read a row
                        finStream.ReadU8Array(rowBuffer);

                        int dstRowPosition = rowStride * y;
                        int srcRowPosition = 0;

                        for (int x = 0; x < width; ++x)
                        {
                            imageBuffer[dstRowPosition + 0] = rowBuffer[srcRowPosition + 2];
                            imageBuffer[dstRowPosition + 1] = rowBuffer[srcRowPosition + 1];
                            imageBuffer[dstRowPosition + 2] = rowBuffer[srcRowPosition + 0];
                            imageBuffer[dstRowPosition + 3] = 0xFF;

                            dstRowPosition += 4;
                            srcRowPosition += bytesPerPixel;
                        }
                    }
                    break;

                default:
                    throw new Exception($"Unsupported BMP Pixel Format: {{ bpp = {bitsPerPixel} }}");
            }
        }
